...
export default function TextInput({id = "", value = "", placeholder, label, changedCallback = (event: ChangeEvent<HTMLInputElement>) => {}} : {id?: string, value?: string, placeholder: string, label: string, changedCallback?: (event: ChangeEvent<HTMLInputElement>) => void}) {
    ...
    return (
        <div className="">
            ...
            <input onChange={changedCallback} id={derivedID} className="text-xs outline-none p-2.5 border border-solid border-cadet_gray-300 rounded-my mt-5 w-96 placeholder:text-xs placeholder:text-cadet_gray-600" type="text" placeholder={placeholder} />
        </div>
    )
}